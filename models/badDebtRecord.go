...
type BadDebtRecord struct {
	...
	DebtAmount       float64 `json:"debt_amount" gorm:"debt_amount"`               // 坏账金额（应扣除但无法扣除的金额）
	UserBalance      float64 `json:"user_balance" gorm:"user_balance"`             // 用户当前余额
	DeficitAmount    float64 `json:"deficit_amount" gorm:"deficit_amount"`         // 缺口金额（负余额的绝对值）
	...
	RecoveryAmount   float64 `json:"recovery_amount" gorm:"recovery_amount"`       // 已回收金额
	WriteOffAmount   float64 `json:"write_off_amount" gorm:"write_off_amount"`     // 核销金额
	RemainingAmount  float64 `json:"remaining_amount" gorm:"remaining_amount"`     // 剩余欠款金额
